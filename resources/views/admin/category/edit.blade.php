                ...
                <div class="col-lg-6 offset-3">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <hr>
                            <form action="{{route('store#Category',$category->id)}}" method="post" novalidate="novalidate">
                                {{csrf_field()}}
                                {{method_field('post')}}
                                <div class="form-group">
                                    <label for="name" class="control-label mb-1">Name</label>
                                    <input id="name" name="name" type="text" class="form-control {{$errors->has('name') ? 'is-invalid' : ''}}"
                                     aria-required="true" aria-invalid="false" placeholder="category name" value="{{$category->name}}">
